...
namespace PhotoSW.Cache.MasterDataCache
    ...
    public class SemiOrderSettingsCaches : HashTableCache<List<SemiOrderSettings>>, ICacheRepository
		...
		public SemiOrderSettingsCaches ()
            {
            this.strPl = SemiOrderSettingsCaches.getPU(400);
            }

        public object GetData ()
            {
            List<SemiOrderSettings> list;
            while(true)
                {
                list = null;
                while(true)
					...
					while(true)
                        ...
                        if(!false)
                            ...
                            if(arg_11_0)
                                {
                                list = new List<SemiOrderSettings>();
                                }
                            else
                                {
                                list = new SemiOrderSettingsDao().GetSemiOrderSettings();
                                if(true)
                                    {
                                    goto Block_4;
                                    }
        ...
        //static SemiOrderSettingsCaches ()
        //    {
        //    // Note: this type is marked as 'beforefieldinit'.
        //    SmartAssembly.HouseOfCards.Strings.CreateGetStringDelegate(typeof(SemiOrderSettingsCaches));
        //    }
        }
    }
